...
const coaching = useSelector((state) => state.UserReducer.afficheCours)
  ...
  return (
    <div>
    <div  style={{fontFamily:' Satisfy', fontSize:'50px', paddingLeft : "15px" }}>Les cours disponibles sont les suivants: </div>
    {coaching?.map((el, key) =>( <div className="cardMap" > <Card>
      <Card.Body  >
        <Card.Title>
          <h5>{el.title}</h5>
          <h3>{el.date}</h3>
  ...
  <Modal show={show} onHide={handleClose}>
        ...
        <Form className="MODALDEVIS">
         ...
         <Form.Group className="mb-3" controlId="formtext">
           ...
           <Form.Control type="text" placeholder="Entrer Prenom" onChange={(e)=> setPrenom(e.target.value)}/>
         </Form.Group>
         <Form.Group className="mb-3" controlId="formtext">
